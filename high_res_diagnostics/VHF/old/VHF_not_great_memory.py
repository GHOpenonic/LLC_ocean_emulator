...
from xarray.conventions import T
import zarr
import xgcm 
import xrft
...
def main():
   ...
   # set size of tile in degrees lat/lon, sets FFT tile sizes (originally box)
    ...
    # j_0 = 620
    # # j_1 = 870
    # chunk_factor = 5 # rechunk graphs by ndeg
...
#    fluctuations = False # just fields
    ...
    LLC_sub = LLC_full.isel(time = slice(t_0,t_1), 
                            i = slice(i_0, i_1), 
                            j = slice(j_0, j_1), face = 7).chunk({'time': -1, 'i': int((i_1-i_0)/chunk_factor), 'j': int((j_1-j_0)/chunk_factor)})
    ...
    # define WT_tiles, W_tiles, T_tiles, weighting by surface cell areas
    A = LLC_sub['rA']
    WT_tiles = ((W * T * A).groupby("box").sum() / A.groupby("box").sum())
    valid = WT_tiles.box != -1
    WT_tiles = WT_tiles.where(valid)
    W_tiles = ((W * A).groupby("box").sum() / A.groupby("box").sum()).where(valid)
    T_tiles = ((T * A).groupby("box").sum() / A.groupby("box").sum()).where(valid)
    ...
    C_p, rho = 3900, 1025
    total_WT_ = (WT_tiles.mean(dim="box") - W_tiles.mean(dim="box") * T_tiles.mean(dim="box"))
    ...
    W_tiles = W_tiles.assign_coords(
        time=((W_tiles.time - W_tiles.time[0]) / np.timedelta64(1, 'h')))
    T_tiles = T_tiles.assign_coords(
        time=((T_tiles.time - T_tiles.time[0]) / np.timedelta64(1, 'h')))
    W_prime = (W_tiles - W_tiles.mean(dim="time"))
    T_prime = (T_tiles - T_tiles.mean(dim="time"))
    ...
    LF_VHF =  C_p * rho * (WT_LF.mean(dim="box") - W_LF.mean(dim="box") * T_LF.mean(dim="box"))
    HF_VHF =  C_p * rho * (WT_HF.mean(dim="box") - W_HF.mean(dim="box") * T_HF.mean(dim="box"))
